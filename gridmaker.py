...
atom_min_dist = 1 # Set minimal distance to an atom in Angstrom
...
for atom in atoms:
    ...
    grid_atom_min_filtered = [x for x,y in zip(grid_atom_min_filtered,distances) if y > 1]
...
pore_dist = 1 # Value from which onwards we consider things to be in a pore - choose this wisely based on the MOF at hand (might need to automate this based on diameter at some point)
